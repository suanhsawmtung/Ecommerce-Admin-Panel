...
export default {
    ...
    mutations: {
        ...
        addNewProduct: (state, newProduct) => {
            newProduct.image = "https://nooneuse.shop/storage/products/" + newProduct.image;
            state.products.unshift(newProduct);
        },

        /* Add Updated Product Data To State */
        updateOldProduct: (state, updatedProduct) => {
            updatedProduct.image = "https://nooneuse.shop/storage/products/" + updatedProduct.image;
            state.products = state.products.filter(product => {
    ...
    actions: {
        ...
        deleteProduct: async({ commit }, removeId) => {
            await axios.delete(`https://nooneuse.shop/api/admin/product/deleteProduct/${removeId}`);
            commit('removeProduct', removeId);
        },

        /* Create New Product */
        createProduct: async({ commit }, newProduct) => {
            let { data } = await axios.post("https://nooneuse.shop/api/admin/product/createProduct", newProduct);
        ...
        updateProduct: async({ commit }, updateData) => {
            try {
                let { data } = await axios.post(`https://nooneuse.shop/api/admin/product/updateProduct`, updateData);
        ...
        searchProduct: async({commit}, searchKey) => {
            const { data } = await axios.get(`https://nooneuse.shop/api/admin/product/getAllProducts/${searchKey}`);
            commit('setProducts', data);
        },

        /* Filter Products By Category */
        filterProductsByCategory: async({commit}, id) => {
            const { data } = await axios.get(`https://nooneuse.shop/api/admin/product/filterProducts/${id}`);
            commit('setProducts', data);
        }
    }
}