...
export default {
    ...
    actions: {
        ...
        allAdmins: async({ commit }) => {
            let { data } = await axios.get("http://localhost:8000/api/user/getAllAdmins");
            commit("setAdmins", data);
        },

        /* Get Paginated Customer Data */
        allCustomers: async({commit}, url) => {
            let URL = url? url : "http://localhost:8000/api/user/getAllCustomers";
        ...
        changeUserRole: async({ commit }, newRoleData) => {
            let { data } = await axios.post('http://localhost:8000/api/user/changeRole', newRoleData);
            commit("addChangedRole", data);
        },

        /* Search Users */
        searchUser: async({ commit }, searchKey) => {
            const { data } = await axios.get(`http://localhost:8000/api/user/getAllCustomers/${searchKey}`);
        ...
        myProfile: async({commit}) => {
            let { data } = await axios.get('http://localhost:8000/api/user/getMyProfile/'+localStorage.getItem('ID'));
